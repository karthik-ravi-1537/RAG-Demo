...
class HierarchicalRAG(BaseRAG):
    ...
    def add_documents(self, document_paths: List[str]) -> None:
        ...
        try:
            ...
            self.stats['hierarchies_created'] += len([h for h in self.hierarchies.values()])
    ...
    def retrieve(self, query: str, top_k: int = None) -> List[Chunk]:
        ...
        try:
            ...
            logger.info(f"Hierarchical retrieval: {len(summary_nodes)} summaries -> {len(detailed_chunks)} chunks in {retrieval_time:.3f}s")
    ...
    def _find_best_section(self, chunk: Chunk, section_nodes: List[HierarchicalNode]) -> Optional[HierarchicalNode]:
    ...
    def _retrieve_summary_nodes(self, query: str, top_k: int) -> List[HierarchicalNode]:
        ...
        summary_nodes = [
            node for node in self.all_nodes.values()
            if node.level in self.summary_levels and node.embedding is not None
        ]
    ...
    def _get_parent_context(self, chunk: Chunk, hierarchy: Optional[HierarchicalStructure]) -> Optional[str]:
    ...
    def _generate_hierarchical_response(self, query: str, context: str, chunks: List[Chunk]) -> str:
        ...
        response_parts = [
            f"Based on the hierarchical analysis of {len(set(chunk.source_document for chunk in chunks))} document(s), here's what I found regarding: '{query}'",
            "",
    ...
    def _calculate_confidence(self, chunks: List[Chunk]) -> float:
        ...
        hierarchy_factor = len(set(chunk.source_document for chunk in chunks)) / max(len(self.hierarchies), 1)
