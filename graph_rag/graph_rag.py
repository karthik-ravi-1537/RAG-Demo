...
from graph_rag.knowledge_graph import KnowledgeGraphBuilder, KnowledgeGraph
...
class GraphRAG(BaseRAG):
        ...
        try:
            ...
            for doc in new_documents:
                ...
                try:
                    ...
                    logger.debug(f"Extracted {len(extraction_result.entities)} entities and "
            ...
            logger.info(f"Successfully added {len(new_documents)} documents to GraphRAG with "
    ...
    def retrieve(self, query: str, top_k: int = None) -> List[Chunk]:
        ...
        try:
            ...
            ranked_chunks = self._rank_chunks_with_graph_context(query, candidate_chunks, query_entities, related_entities)
            ...
            logger.info(f"Graph-RAG retrieval: {len(query_entities)} query entities -> "
    ...
    def _build_knowledge_graph(self, extraction_results: List[ExtractionResult]) -> None:
        ...
        logger.info(f"Built knowledge graph with {len(all_entities)} entities and {len(all_relationships)} relationships")
    ...
    def _find_similar_entities(self, query: str, top_k: int = 3) -> List[str]:
        ...
        similarities = self.embedding_system.compute_similarity(
            query_embedding.reshape(1, -1), entity_embeddings
        )
    ...
    def _traverse_graph_for_entities(self, query_entities: List[str], query: str) -> List[Tuple[str, float]]:
        ...
        for query_entity in query_entities:
            ...
            direct_related = self.knowledge_graph.find_related_entities(
                query_entity, max_depth=2, relation_types=None
            )
    ...
    def _rank_chunks_with_graph_context(self, query: str, candidate_chunks: List[Tuple[Chunk, float]], 
                                       query_entities: List[str], related_entities: List[Tuple[str, float]]) -> List[Chunk]:
        ...
        for chunk, graph_score in candidate_chunks:
            ...
            combined_score = (0.4 * similarity_score + 
                            0.4 * graph_score + 
    ...
    def _calculate_connectivity_score(self, chunk: Chunk, query_entities: List[str], 
                                    related_entities: List[Tuple[str, float]]) -> float:
        ...
        for chunk_entity in chunk_entities:
            ...
            for query_entity in query_entities:
                ...
                paths = self.knowledge_graph.find_connection_paths(
                    chunk_entity, query_entity, max_length=3
                )
                ...
                if paths:
                    ...
                    avg_confidence = sum(
                        rel['confidence'] for rel in shortest_path['relationships'] 
                        if rel and 'confidence' in rel
    ...
    def _generate_graph_response(self, query: str, context: str, chunks: List[Chunk]) -> str:
        ...
        response_parts = [
            f"Based on knowledge graph analysis of {len(set(chunk.source_document for chunk in chunks))} document(s), "
    ...
    def _explain_process(self, query: str, chunks: List[Chunk]) -> Dict[str, Any]:
        ...
        score_analysis = {
            'avg_semantic_similarity': sum(
                chunk.metadata.get('semantic_similarity', 0) for chunk in chunks
            ) / max(len(chunks), 1),
            'avg_graph_relevance': sum(
                chunk.metadata.get('graph_relevance', 0) for chunk in chunks
            ) / max(len(chunks), 1),
            'avg_connectivity': sum(
                chunk.metadata.get('connectivity_score', 0) for chunk in chunks
            ) / max(len(chunks), 1)
        }
