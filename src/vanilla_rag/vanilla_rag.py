...
from core.data_models import Chunk, ProcessedDocument, RAGConfig, RAGResult
...
class VanillaRAG(BaseRAG):
    ...
    def add_documents(self, document_paths: list[str]) -> None:
        ...
        try:
            ...
            logger.info(f"Successfully added {len(new_documents)} documents, " f"created {len(embedded_chunks)} chunks")
    ...
    def retrieve(self, query: str, top_k: int = None) -> list[Chunk]:
        ...
        try:
            ...
            self.embedding_system.compute_similarity(query_embedding, self.chunk_embeddings)
    ...
    def _explain_process(self, query: str, chunks: list[Chunk]) -> dict[str, Any]:
        ...
        return {
            ...
            "top_similarities": [chunk.similarity_score for chunk in chunks[:5] if chunk.similarity_score > 0],
            "source_documents": list({chunk.source_document for chunk in chunks if chunk.source_document}),
            "process_steps": [
