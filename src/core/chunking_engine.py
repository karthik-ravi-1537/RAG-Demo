...
from utils.text_utils import count_tokens, extract_sentences

from core.data_models import Chunk, ChunkingConfig, ProcessedDocument
        ...
        for line in lines:
            ...
            if header_pattern.match(line):
                ...
                if current_length + line_length > config.chunk_size and current_section:
                    ...
                    current_length = sum(len(line) for line in current_section)
                else:
...
class ChunkingEngine(BaseChunker):
    ...
    def get_chunking_stats(self, chunks: list[Chunk]) -> dict[str, Any]:
        ...
        stats = {
            ...
            "strategies_used": list({chunk.metadata.get("chunking_strategy", "unknown") for chunk in chunks}),
        }
