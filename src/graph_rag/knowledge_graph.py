...
from core.data_models import Entity, Relationship
from core.data_models import KnowledgeGraph as KGModel
...
class KnowledgeGraphBuilder:
    ...
    def __init__(self):
        ...
        if nx is None:
            raise ImportError(
                "NetworkX is required for knowledge graph functionality. Install with: pip install networkx"
            )
    ...
    def add_relationships(self, relationships: list[Relationship]) -> None:
        ...
        for relationship in relationships:
            ...
            if (
                relationship.source_entity not in self.entity_index
                or relationship.target_entity not in self.entity_index
            ):
                logger.warning(
                    f"Skipping relationship with missing entities: {relationship.source_entity} -> {relationship.target_entity}"
                )
                continue
    ...
    def get_neighbors(self, entity_name: str, max_depth: int = 2, relation_types: list[str] | None = None) -> list[str]:
    ...
    def find_paths(self, source: str, target: str, max_length: int = 3) -> list[list[str]]:
        ...
        try:
            ...
            paths = list(nx.all_simple_paths(self.graph.to_undirected(), source, target, cutoff=max_length))
            return paths
    ...
    def save_graph(self, file_path: str | Path, format: str = "pickle") -> None:
        ...
        try:
            if format == "pickle":
                ...
                graph_data = {
                    "nodes": [{"id": node, "attributes": data} for node, data in self.graph.nodes(data=True)],
                    "edges": [
                        {"source": source, "target": target, "attributes": data}
    ...
    def load_graph(self, file_path: str | Path, format: str = "pickle") -> None:
...
class KnowledgeGraph:
    ...
    def query_entities(self, entity_type: str | None = None, min_confidence: float = 0.0) -> list[Entity]:
    ...
    def query_relationships(self, relation_type: str | None = None, min_confidence: float = 0.0) -> list[Relationship]:
    ...
    def find_related_entities(
        self, entity_name: str, max_depth: int = 2, relation_types: list[str] | None = None
    ...
    def find_connection_paths(
        self, source_entity: str, target_entity: str, max_length: int = 3
    ) -> list[dict[str, Any]]:
        ...
        detailed_paths = []
        for path in paths:
            path_details = {"entities": path, "length": len(path) - 1, "relationships": []}
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            **self.builder.stats,
            ...
            "avg_entity_confidence": sum(e.confidence for e in self.model.entities.values())
            / max(len(self.model.entities), 1),
            "avg_relationship_confidence": sum(r.confidence for r in self.model.relationships)
            / max(len(self.model.relationships), 1),
        }
