...
from core.data_models import Chunk, Entity, ProcessedDocument, Relationship
...
class EntityExtractor:
    ...
    def extract_from_chunk(self, chunk: Chunk) -> ExtractionResult:
        ...
        try:
            ...
            logger.debug(
                f"Extracted {len(entities)} entities and {len(relationships)} relationships from chunk {chunk.id}"
            )

            return ExtractionResult(entities=entities, relationships=relationships, extraction_stats=stats)
    ...
    def extract_from_document(self, document: ProcessedDocument, chunks: list[Chunk]) -> ExtractionResult:
        ...
        try:
            ...
            logger.info(
                f"Document {document.id}: {len(merged_entities)} entities, {len(filtered_relationships)} relationships"
            )

            return ExtractionResult(
                entities=merged_entities, relationships=filtered_relationships, extraction_stats=dict(combined_stats)
            )
    ...
    def _extract_capitalized_entities(self, text: str, chunk_id: str) -> list[Entity]:
        ...
        for match in matches:
            ...
            if (
                len(entity_text) > 3
                and entity_text not in ["The", "This", "That", "These", "Those", "When", "Where", "What", "How"]
                and not entity_text.startswith(("Figure", "Table", "Section", "Chapter"))
            ):
                ...
                for match in matches:
                    ...
                    if source_entity and target_entity and source_entity != target_entity:
                        ...
                        relationship = Relationship(
                            ...
                            properties={"indicator": indicator, "context": match.group(), "position": match.span()},
                        )
                ...
                if distance < 200:  # Within 200 characters
                    ...
                    if rel_type:
                        ...
                        relationship = Relationship(
                            ...
                            confidence=confidence,
                            evidence_chunks=[chunk_id],
                            properties={"extraction_method": "proximity", "distance": distance, "context": context},
                        )
                        relationships.append(relationship)
            ...
            if key not in seen and reverse_key not in seen:
                ...
                existing_rel = next(
                    (
                        ...
                        or (r.target_entity, r.source_entity, r.relation_type) == key
                    ),
                    None,
                )
    ...
    def _calculate_relationship_confidence(
        self, source: Entity, target: Entity, rel_type: str, indicator: str, context: str
    ) -> float:
    ...
    def _find_matching_entity(self, text: str, entities: list[Entity]) -> Entity | None:
    ...
    def _infer_relationship_type(self, entity1: Entity, entity2: Entity, context: str) -> str | None:
