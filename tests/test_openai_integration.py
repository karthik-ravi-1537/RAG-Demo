...
from core.exceptions import EmbeddingError, APIError
...
class TestOpenAIIntegration:
    ...
    def test_openai_api_call(self):
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "user", "content": "Say 'API test successful' exactly."}
                ],
                max_tokens=10,
                temperature=0
            )
    ...
    def test_rag_with_openai_embeddings(self):
        ...
        try:
            ...
            print(f"✅ RAG with OpenAI embeddings: {len(result.retrieved_chunks)} chunks, confidence={result.confidence_score:.3f}")
    ...
    def test_api_key_validation_format(self):
        ...
        if api_key and api_key != "test_key_placeholder":
            ...
            for indicator in placeholder_indicators:
                assert indicator.lower() not in api_key.lower(), f"API key appears to contain placeholder text: {indicator}"
        else:
