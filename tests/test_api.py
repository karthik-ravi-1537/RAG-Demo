...
def test_openai_api():
    ...
    if not api_key or api_key == "test_key_placeholder":
        ...
        raise AssertionError("No valid OpenAI API key found")
    ...
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "Say 'Hello from RAG Demo!' in exactly those words."}],
        max_tokens=20,
        temperature=0,
    )
...
def test_rag_with_llm():
    ...
    print("✅ RAG with LLM test successful!")
