...
from evaluation.rag_comparator import RAGComparator, ComparisonQuery
...
def create_test_queries() -> List[ComparisonQuery]:
    ...
    return [
        ...
        ComparisonQuery(
            id="q5",
            query="What role do transformers play in modern NLP?",
            category="technical",
            difficulty="advanced"
        )
    ]
...
def main():
    ...
    try:
        ...
        print(f"Hierarchical RAG: {hierarchical_stats['documents_processed']} docs, {hierarchical_stats['total_nodes']} nodes")
        
        graph_stats = graph_rag.get_stats()
        print(f"Graph RAG: {graph_stats['documents_processed']} docs, {graph_stats['entities_extracted']} entities, {graph_stats['relationships_extracted']} relationships")
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            if comparison_result.comparison_summary:
                ...
                if perf_summary:
                    print(f"\n⚡ Speed Analysis:")
                    print(f"   Fastest: {perf_summary.get('fastest_system')} ({perf_summary.get('fastest_time', 0):.3f}s)")
                    print(f"   Slowest: {perf_summary.get('slowest_system')} ({perf_summary.get('slowest_time', 0):.3f}s)")
                
                quality_summary = comparison_result.comparison_summary.get('quality_summary', {})
                if quality_summary:
                    print(f"   Quality Leader: {quality_summary.get('highest_confidence_system')} ({quality_summary.get('highest_confidence_score', 0):.3f})")
        ...
        print(f"   Vanilla RAG: Fast, simple, good baseline performance")
        print(f"   Hierarchical RAG: Structure-aware, good for complex documents")
        print(f"   Graph RAG: Relationship-aware, best for connected information")
        
        print(f"\n✨ Demo completed successfully!")
        print(f"\n💡 Key Insights:")
        print(f"   - Different RAG approaches excel in different scenarios")
        print(f"   - Graph RAG provides richer context through entity relationships")
        print(f"   - Hierarchical RAG better handles document structure")
        print(f"   - Vanilla RAG offers good performance with simplicity")
        print(f"   - Choice depends on your specific use case and requirements")
