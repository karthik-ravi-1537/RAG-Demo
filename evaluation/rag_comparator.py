...
from core.data_models import RAGResult, RAGConfig
...
class RAGComparator:
    ...
    def compare_single_query(self, query: Union[str, ComparisonQuery], 
                           rag_names: Optional[List[str]] = None) -> ComparisonResult:
    ...
    def compare_batch_queries(self, queries: List[Union[str, ComparisonQuery]], 
                            rag_names: Optional[List[str]] = None) -> List[ComparisonResult]:
    ...
    def get_aggregate_analysis(self, rag_names: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    def generate_comparison_report(self, output_path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
    ...
    def export_results(self, output_dir: Union[str, Path], format: str = 'json') -> None:
    ...
    def _analyze_comparison(self, results: Dict[str, RAGResult], 
                          metrics: Dict[str, RAGPerformanceMetrics]) -> Dict[str, Any]:
    ...
    def _determine_winner(self, metrics: Dict[str, RAGPerformanceMetrics]) -> Optional[str]:
        ...
        # Scoring system: 40% confidence, 30% speed, 20% efficiency, 10% response quality
        scores = {}
        ...
        for name, m in valid_metrics.items():
            ...
            # Confidence score (40%)
            score += 0.4 * m.confidence_score
            
            # Speed score (30%) - inverse of time, normalized
            max_time = max(vm.total_time for vm in valid_metrics.values())
            if max_time > 0:
                speed_score = 1.0 - (m.total_time / max_time)
                score += 0.3 * speed_score
            
            # Efficiency score (20%) - confidence per chunk
            if m.chunks_retrieved > 0:
                efficiency_score = m.confidence_score / m.chunks_retrieved
                max_efficiency = max(vm.confidence_score / max(vm.chunks_retrieved, 1) 
                                   for vm in valid_metrics.values())
                if max_efficiency > 0:
                    score += 0.2 * (efficiency_score / max_efficiency)
            
            # Response quality score (10%) - based on response length (moderate length preferred)
            optimal_length = 500  # Optimal response length
            length_score = 1.0 - abs(m.response_length - optimal_length) / optimal_length
    ...
    def _generate_rankings(self) -> Dict[str, Any]:
        ...
        for rag_name, rag_analysis in analysis.items():
            ...
            if rag_analysis['total_times']['count'] > 0:
                ...
                max_time = max(a['total_times']['mean'] for a in analysis.values() 
                             if a['total_times']['count'] > 0)
                if max_time > 0:
        ...
        rankings['overall'] = {name: {'rank': i+1, 'score': score} 
                             for i, (name, score) in enumerate(sorted_overall)}
