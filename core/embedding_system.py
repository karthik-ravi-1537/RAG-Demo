...
from core.data_models import EmbeddingConfig, Chunk
from core.exceptions import EmbeddingError, APIError, ModelNotFoundError
...
class SentenceBERTProvider(EmbeddingProvider):
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        ...
        try:
            ...
            raise ModelNotFoundError("sentence-transformers library not installed. Install with: pip install sentence-transformers")
        except Exception as e:
...
class EmbeddingSystem(BaseEmbedder):
    ...
    def find_most_similar(self, query_embedding: np.ndarray, doc_embeddings: np.ndarray, top_k: int = 5) -> Tuple[np.ndarray, np.ndarray]:
